...
export const metadata: Metadata = {
  ...
  description: "Présentation des projets professionnels pour l'épreuve E6 du BTS SIO."
};
...
export default async function E6Page() {
  ...
  const e6Projects = projects.filter((p: Project) => p.id === 5 || p.id === 4);
      ...
      <section className="space-y-8">
        ...
        <header className="space-y-3">
          ...
          <p className="text-muted-foreground max-w-xl">
            ...
            application mobile de prise de notes en React Native et une application desktop de
            gestion d'interventions matérielles en C#.
          </p>
          <div className="pt-1">
            <Button
              size="sm"
              variant="outline"
              className="border-[#043c85]/30 text-[#043c85] hover:bg-[#043c85]/5 dark:text-blue-300"
              asChild>
              <Link href="/stage/print" target="_blank">
                <FileTextIcon className="mr-1.5 size-3.5" />
                Fiche projet de stage (PDF)
              </Link>
            </Button>
          </div>
        </header>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          {e6Projects.map((project: Project, i: number) => (
